...
class MyButton:
    def __init__(self, root, canvas, label, type):
        ...
        self.label = label
        if type == 0:
            button = Tkinter.Button(root, text = 'Line', command = self.DrawLine)
        elif type == 1:
            button = Tkinter.Button(root, text = 'Arc', command = self.DrawArc)
        elif type == 2:
            button = Tkinter.Button(root, text = 'Rec', command = self.DrawRec)
        else:
            button = Tkinter.Button(root, text = 'Oval', command = self.DrawOval)
        button.pack(side = 'left')
    def DrawLine(self):
        self.label.text.set('Line')
        self.canvas.SetStatus(0)
    def DrawArc(self):
        self.label.text.set('Arc')
        self.canvas.SetStatus(1)
    def DrawRec(self):
        self.label.text.set('Rec')
        self.canvas.SetStatus(2)
    def DrawOval(self):
        self.label.text.set('Oval')
